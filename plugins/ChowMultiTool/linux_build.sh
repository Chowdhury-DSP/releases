...
# build 64-bit
cmake -Bbuild -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=clang-12 -DCMAKE_CXX_COMPILER=clang++-12
cmake --build build --config Release --parallel 4 --target ChowMultiTool_Standalone ChowMultiTool_VST3 ChowMultiTool_CLAP ChowMultiTool_LV2
...
Description: Multi-Tool audio plugin.
 ChowMultiTool includes VST3, LV2, CLAP, and Standalone formats.
EOT
...
${name} (${version}) stable; urgency=medium
  ...
  * ${MSG}
  * For more details see https://github.com/Chowdhury-DSP/ChowMultiTool
...
# cp -R build/ChowMultiTool_artefacts/Release/VST/ChowMultiTool.so "${name}/usr/lib/vst/"
cp -R build/ChowMultiTool_artefacts/Release/VST3/ChowMultiTool.vst3 "${name}/usr/lib/vst3/"
cp -R build/ChowMultiTool_artefacts/Release/LV2/ChowMultiTool.lv2 "${name}/usr/lib/lv2/"
cp -R build/ChowMultiTool_artefacts/Release/CLAP/ChowMultiTool.clap "${name}/usr/lib/clap/"
cp -R build/ChowMultiTool_artefacts/Release/Standalone/ChowMultiTool "${name}/usr/bin/"
...
chmod -R 0755 "${name}/usr/bin/ChowMultiTool"
